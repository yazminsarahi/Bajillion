...
class SearchServer:
    def __init__(self):
        ...
        self.all_document_lines = create_index(self.filenames, self.index, self.file_titles)
    ...
    def handle_request(self, request):
        ...
        if request.command == 'search':
            ...
            query = params['query']
            results = search(self.index, query)
            title_results = get_title_results(results, self.file_titles, self.all_document_lines)
            return json.dumps(title_results, indent=2)

def get_title_results(results, file_titles, all_document_lines):
    """
    This function is given a dictionary mapping from
    file names to the titles of articles in those files and search results.
    It returns a list of dictionaries of each result title.
    """
    titles_result = []
    for result in results:
        ...
        #url = os.path.join(directory, DIRECTORY)
        url = ''
        url = os.path.join(url, result)
        start_snippet = len(title.split())
        end_snippet = 2 + len(title)
        snippet_list = all_document_lines[result].split()[start_snippet:end_snippet]
        snippet_text = ' '.join(snippet_list)

        snippet_text += " ..."
        titles_result.append({"title" : title, "url": url, "snippet": snippet_text, "date": "01/01/2023"})
