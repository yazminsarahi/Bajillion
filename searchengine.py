...
#from extension import sort_by_ranking
...
def create_index(filenames, index, file_titles):
    ...
    #if the filenames is an empty list we are gonna get the first txt file in the directory
    if not filenames:
        filenames = get_first_filename_in_dir()

    #Map the filename with all the document lines in the txt file
    for filename in filenames:
        get_document_title_lines(filename, file_titles, all_document_lines)

    #parse all document terms for each txt file
    for filename, document_lines in all_document_lines.items():
        document_terms = parse(document_lines)
        for term in document_terms:
            if term not in index:
                index[term] = [filename]
            elif filename not in index[term]:
    ...
    """
def get_first_filename_in_dir():
    directory = Path().absolute()
    files = textfiles_in_dir(directory)
    filename = files[0].split("/")
    filename = [filename[-1]]
    return filename
def get_document_title_lines(filename, file_titles, all_document_lines):
    with open(filename) as f:
        file_title = f.readline()  # stores first line of file in first_line
        file_titles[filename] = file_title.strip()
        all_document_lines[filename] = file_title
        for line in f:  # loops over remaining lines of file
            all_document_lines[filename] += line
...
def search(index, query):
    ...
    []
    """
    list_index_query = get_list_index_query(query, index)

    previous_list_index_query = list_index_query[0] if list_index_query else []
    common_list_index_query = previous_list_index_query

    for current_list_index_word in list_index_query[1:]:
        common_list_index_query = common_elements.common(previous_list_index_query, current_list_index_word)
        previous_list_index_query = common_list_index_query
    return  common_list_index_query
    """
    You implement this function.  Don't forget to remove the 'pass' statement above.
    """
def get_list_index_query(query, index):
    list_index_query = []
    for word in query.split():
        if word in index:
            list_index_query.append(sorted(index[word]))
        else:
            list_index_query.append([])
    return list_index_query
...
def main():
    ...
    if num_args < 1 or num_args > 2:
        ...
        if os.path.exists(directory):
            ...
            all_document_lines = create_index(files, index, file_titles)
